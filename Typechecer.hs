...
typeCheckF environment (Fun tR id decls stms) =
    if (tR /= Tvoid)
        then
            if (stms /= [])
                then
                    let lastCommand = last stms
                     in case lastCommand of
                            SReturn exp ->
                                let r = tk newEnvironment (SBlock stmsButLast)
                                 in case r of
                                        OK env -> tke env exp tR
                                        Erro msg -> Erro msg
                              where
                                stmsButLast = reverse (tail (reverse stms))
  ...
  where
    typeBindings = map (\(Dec tp id) -> (id, tp)) decls
...
tinf environment x = case x of
    ...
    Call id lexp ->
        let r = lookupShallowFunction environment id
         in case r of
                OK (TFun (Fun tipoRetorno _ decls stms)) ->
...
updateShallowType ((s : sc), fnCtx) id tp =
    let r = updateShallow s id tp
     in case r of
